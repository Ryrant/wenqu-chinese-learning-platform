...
export async function GET(request: Request) {
  try {
    ...
    const submissionReviewsQuery = (admin || roles.includes("teacher")) ? db.prepare(`SELECT sr.* FROM submission_reviews sr JOIN submissions s ON s.id=sr.submission_id AND s.tenant_id=sr.tenant_id JOIN assignments a ON a.id=s.assignment_id AND a.tenant_id=s.tenant_id JOIN classes c ON c.id=a.class_id AND c.tenant_id=a.tenant_id WHERE sr.tenant_id=? AND ${submissionAccess.sql} ORDER BY sr.created_at DESC LIMIT 50`).bind(tenantId, ...submissionAccess.args) : null;
    const [classes, assignments, submissions, submissionReviews, mastery, documents, plans, notifications, consents, audits, invitations, members, guardianLinks] = await Promise.all([
      classesQuery.all(), assignmentsQuery.all(), submissionsQuery.all(), submissionReviewsQuery ? submissionReviewsQuery.all() : Promise.resolve(empty),
      ...
      admin || roles.includes("teacher") ? db.prepare(`SELECT d.*,d.processing_error AS processingError,COUNT(k.id) AS chunkCount,SUM(CASE WHEN k.published=1 THEN 1 ELSE 0 END) AS publishedChunkCount FROM source_documents d LEFT JOIN knowledge_chunks k ON k.source_document_id=d.id AND k.tenant_id=d.tenant_id WHERE d.tenant_id=? GROUP BY d.id ORDER BY d.created_at DESC`).bind(tenantId).all() : Promise.resolve(empty),
