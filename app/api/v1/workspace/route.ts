...
export async function GET(request: Request) {
  try {
    ...
    const submissionReviewsQuery = db.prepare(`SELECT sr.* FROM submission_reviews sr JOIN submissions s ON s.id=sr.submission_id AND s.tenant_id=sr.tenant_id JOIN assignments a ON a.id=s.assignment_id AND a.tenant_id=s.tenant_id JOIN classes c ON c.id=a.class_id AND c.tenant_id=a.tenant_id WHERE sr.tenant_id=? AND ${submissionAccess.sql} ORDER BY sr.created_at DESC LIMIT 50`).bind(tenantId, ...submissionAccess.args);
    const empty = { results: [] };
    const [classes, assignments, submissions, submissionReviews, mastery, documents, plans, notifications, consents, audits, invitations, members, guardianLinks] = await Promise.all([
      classesQuery.all(), assignmentsQuery.all(), submissionsQuery.all(), submissionReviewsQuery.all(),
